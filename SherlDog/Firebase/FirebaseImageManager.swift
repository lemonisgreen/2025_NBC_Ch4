...
extension FirebaseImageManager {
    ...
    /// detectiveMate 이미지 업로드
    func uploadDetectiveMateImage(_ image: UIImage, completion: @escaping (Result<String, Error>) -> Void) {
        guard let userId = Auth.auth().currentUser?.uid else {
            completion(.failure(ImageError.invalidImageData)); return
        }
        let uuid = UUID().uuidString
        let path = "detectiveMate/\(userId)/detectiveMate_\(uuid).jpg"
        uploadImageToPath(image, path: path, completion: completion)
    }
